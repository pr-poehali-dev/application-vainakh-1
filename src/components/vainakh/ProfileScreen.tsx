...
const ProfileScreen = ({ userData, theme, onThemeToggle }: ProfileScreenProps) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      >
        <span className="font-semibold" style={{ color: "var(--text)" }}>Профиль</span>
      ...
      <div className="px-4 py-3 pb-24 space-y-3">
              ...
              <div className="p-4 flex items-center gap-3">
                ...
                <div className="flex gap-0.5 items-end h-5 flex-1">
                  {[3, 5, 8, 4, 7, 6, 9, 5, 3, 7, 4, 6].map((h, i) => (
                    <div key={i} className="w-1 rounded-full" style={{ height: `${h * 2}px`, background: "var(--border-2)" }} />
                  ))}
                </div>
              </div>
            )}
        ...
        {activeTab === "photos" && (
          <div className="grid grid-cols-3 gap-2">
            ...
            >
              <Icon name="Plus" size={20} style={{ color: "var(--text-3)" }} />
            </div>
          </div>
        )}
          ...
          <div className="space-y-2">
            {["Вайнах — в сердце", "Горный ветер", "Распев #1"].map((title, i) => (
              ...
              >
                ...
                <span className="text-xs" style={{ color: "var(--text-3)" }}>🔥 {(i + 1) * 47}</span>
              </div>
            ))}
          </div>
        )}
