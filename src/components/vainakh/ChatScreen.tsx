    ...
    <div className="flex flex-col h-full animate-fade-in">
      ...
      >
        ...
        <button
          ...
          style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
        >
          <Icon name="Phone" size={15} style={{ color: "var(--text)" }} />
        </button>
        <button
          className="w-8 h-8 rounded-lg flex items-center justify-center"
          style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
        >
          <Icon name="Video" size={15} style={{ color: "var(--text)" }} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.fromMe ? "justify-end" : "justify-start"} animate-fade-in`}>
            <div
              className="max-w-xs px-3.5 py-2.5 rounded-2xl relative"
              style={{
                background: msg.fromMe ? "var(--text)" : "var(--surface)",
            ...
            >
              <p
                ...
                style={{ color: msg.fromMe ? "var(--bg)" : "var(--text)" }}
              >
                {msg.text}
              </p>
              <p
                className="text-xs mt-0.5 text-right"
                style={{ color: msg.fromMe ? "rgba(0,0,0,0.4)" : "var(--text-3)" }}
              >
                {msg.time}
              ...
              {msg.reaction && (
                <span
                  className="absolute -bottom-2 -right-1 text-sm px-1 rounded-full border"
        ...
        <div className="flex justify-start">
          <div
            className="max-w-xs px-3.5 py-3 rounded-2xl"
            style={{
  ...
  return (
    <div className="flex flex-col h-full">
      <div
        ...
        style={{ background: "var(--bg)", borderBottom: "1px solid var(--border)" }}
      >
        <span className="font-semibold" style={{ color: "var(--text)" }}>Сообщения</span>
        <button
          className="w-8 h-8 rounded-lg flex items-center justify-center"
          style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
        >
          <Icon name="Edit" size={15} style={{ color: "var(--text)" }} />
        </button>
      </div>
